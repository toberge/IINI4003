...
// From example: int + Fraction, Fraction + int
...
Fraction &Fraction::operator+=(const int number) {
  set(numerator + denominator * number, denominator);
  return *this;
}
...
// TASK: int - Fraction, Fraction - int

// perhaps not the cleanest way to do it
// (I didn't look at the example for `+`)
...
Fraction &Fraction::operator-=(const int number) {
  set(numerator - denominator * number);
  return *this;
}
